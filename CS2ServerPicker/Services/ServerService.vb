...
Module ServerService
    ...
    Public Async Function Fetch_Server_Data() As Task(Of String)
        Try
            Dim webReq As String = Await New WebClient().DownloadStringTaskAsync("https://api.steampowered.com/ISteamApps/GetSDRConfig/v1/?appid=1422450")
        ...
        For Each serverKvp As KeyValuePair(Of String, String) In serverDictionary
            ...
            Try
                proc.StartInfo.Arguments = $"/c {netshPath} advfirewall firewall add rule " +
                        "name=DeadlockServerPicker_" + serverKvp.Key.Replace(" ", "") + " dir=out action=block protocol=ANY " +
        ...
        For Each row As DataGridViewRow In mainDataGridView.SelectedRows
            ...
            Try
                ...
                proc.StartInfo.Arguments = $"/c {netshPath} advfirewall firewall " + If(block, "add", "delete") + " rule " +
                        "name=DeadlockServerPicker_" + region.Replace(" ", "") + If(block, " dir=out action=block protocol=ANY " +
    ...
    Public Function Is_Server_Blocked_Or_Unblocked(region As String, block As Boolean) As Boolean
        ...
        proc.StartInfo.Arguments = $"/c {netshPath} advfirewall firewall show rule name=DeadlockServerPicker_" + region_trimmed
        proc.Start()
        proc.WaitForExit()
        ...
        is_rule_exist = procOutput.Contains("DeadlockServerPicker_" + region_trimmed)
